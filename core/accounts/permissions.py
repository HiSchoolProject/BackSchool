...
from core.model.permissions import ModelPermission


class AccountPermission(ModelPermission):

    def get_allowed_fields_permisssions(self, request, obj):
        if request.user.has_perm('accounts.change_account') or request.user is obj.user:
            return ['user']
        else:
            return []
